...
import { Loader2, FileVideo, FileImage, X, Plus, MicroscopeIcon, ArrowLeft } from "lucide-react";
...
export default function ProjectAnalysisPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (!groups && (data.data?.heuristics || data.heuristics)) {
            const heuristics = data.data?.heuristics || data.heuristics;
            groups = groupHeuristics(heuristics);
          }
          setHeuristicsGroups(groups || []);
        } else {
          setHeuristicsError(data.error || "Erro ao carregar heurísticas.");
        }
      } catch (err) {
        setHeuristicsError(err.message || "Erro ao carregar heurísticas.");
      } finally {
  ...
  const loadEvaluations = useCallback(async (selected) => {
    ...
    const allItems = await db.evaluations.orderBy("id").reverse().toArray();
