...
export default function ProjectAnalysisPage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const heuristicsUrl = baseUrl
                    ? `${baseUrl}/heuristics?project=${encodeURIComponent(slug)}`
                    : `/api/heuristics?project=${encodeURIComponent(slug)}`;
                const headers = {};
                const apiKey = process.env.NEXT_PUBLIC_API_KEY;
                if (apiKey) {
                    headers["api_key"] = apiKey;
                }
                const response = await fetch(heuristicsUrl, { headers });
                const data = await response.json();
                if (response.ok) {
                    let groups = data.groups;
                    // debugger;
                    if (!groups && (data.data?.heuristics || data.heuristics)) {
                        const heuristics =
                            data.data?.heuristics || data.heuristics;
                        groups = groupHeuristics(heuristics);
                    }
                    setHeuristicsGroups(groups || []);
                } else {
                    setHeuristicsError(
                        data.error || "Erro ao carregar heurísticas.",
                    );
                }
            } catch (err) {
                setHeuristicsError(
                    err.message || "Erro ao carregar heurísticas.",
                );
            } finally {
    ...
    const loadEvaluations = useCallback(
        async (selected) => {
            ...
            const allItems = await db.evaluations
                .orderBy("id")
                .reverse()
                .toArray();
            const items = allItems.filter((e) => e.projectSlug === slug);
    ...
    const handleAnalyze = async () => {
        ...
        if (!geminiApiKey) {
            setUploadStatus(
                "Erro: NEXT_PUBLIC_GEMINI_API_KEY não configurada.",
            );
            return;
        }
                ...
                <main className="flex-1 overflow-y-auto">
                    <div className="mx-auto max-w-4xl space-y-8 px-6 py-8">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex flex-col gap-2 flex-1">
                                    <h2 className="text-base font-semibold">
                                        Select the heuristics you want to
                                        investigate
                                    </h2>
                        ...
                        <section className="space-y-3">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex flex-col gap-2 flex-1">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        You can select one or more files by
                                        holding Ctrl / Cmd key.
                                    </p>
                                    <DrivePickerButton
                        ...
                        <section className="flex items-center gap-4 justify-center flex-col">
                            <div className="flex flex-col gap-2 w-full max-w-[600px] items-center">
                                {loading ? (
                                    <Button
                                        disabled
                                        className="gap-2 w-full max-w-[400px] text-lg py-6"
                                    >
                                    ...
                                    <Button
                                        ...
                                        disabled={!canAnalyze}
                                        className="gap-2 w-full max-w-[300px] text-lg py-6"
                                    >
                        ...
                        {activeEvaluation?.results &&
                            activeEvaluation.results.length > 0 && (
                                <>
                                    <Separator />
